        ...
        if($_FILES['insert']['name']){
            ...
            if(end($filename)=="csv"){
                ...
                while($data=fgetcsv($f)) {
                    ...
                    $check_ = mysqli_query($conn, "select id from students where student_reg='$student_reg' and class_id=$class_id ");
                    if($check_==NULL)
                    {$check=0;}
                    else{
                    $check = mysqli_num_rows($check_);}
                    if ($check > 0 ) {
                        array_push($list2, $student_reg);
                        $flag2 = false;
                    } else {
                ...
                if(!$flag2){
                    foreach ($list2 as $t) {
                        echo "This student record already exists :$t <br/>";
                    }
                }
                if($flag){
